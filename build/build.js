...
const monthlyPanel = '<div style="background:var(--panel);border:1px solid var(--border);border-radius:18px;padding:20px;"><div style="font-weight:800;font-family:\'Archivo\';font-size:16px;margin-bottom:16px;">Monitoring Kelas per Bulan · {{ monthlyYear }}</div><div style="display:grid;grid-template-columns:repeat(12,1fr);gap:6px;align-items:end;"><sc-for list="{{ monthly }}" as="m"><div style="text-align:center;"><div style="font-size:13px;font-weight:800;font-family:\'Archivo\';margin-bottom:6px;color:{{ m.col }};">{{ m.count }}</div><div style="height:{{ m.h }}px;min-height:4px;background:{{ m.bar }};border-radius:5px;"></div><div style="font-size:10px;color:var(--muted);margin-top:6px;">{{ m.month }}</div></div></sc-for></div></div>';
const monthlyScreen = '<sc-if value="{{ s.monthly }}"><div style="max-width:980px;margin:0 auto;"><div style="font-family:\'Archivo\';font-weight:800;font-size:22px;margin-bottom:4px;">Monitoring Kelas</div><div style="color:var(--muted);font-size:13.5px;margin-bottom:20px;">Jumlah kelas yang Anda ampu setiap bulan sepanjang tahun.</div>' + monthlyPanel + '</div></sc-if>';
