...
const menuModal = '<sc-if value="{{ showMenuModal }}" hint-placeholder-val="{{ false }}">'
  ...
  + '<div onclick="{{ stopProp }}" style="background:var(--panel2);border:1px solid var(--border2);border-radius:20px;padding:24px;max-width:660px;width:100%;margin:auto;">'
    ...
    + '<sc-for list="{{ mbBlocks }}" as="blk"><div style="' + cardBox + 'padding:12px 13px;margin-bottom:8px;background:var(--panel);">'
      ...
      + '</div>'
    + '</div></sc-for>'
    + '<button onclick="{{ addMenuBlock }}" style="width:100%;background:transparent;border:1px dashed var(--border2);color:var(--muted);border-radius:10px;padding:9px;font-weight:700;font-size:13px;cursor:pointer;margin-bottom:10px;">+ Add Block</button>'
    + '<label style="' + vLabel + '">Notes / manual add-on — after blocks <span style="color:var(--muted2);font-weight:400;">(optional)</span></label>'
    + '<textarea data-mb="noteBottom" rows="2" placeholder="e.g. cooldown, mobility, or extra exercises typed by hand" style="' + mi + 'width:100%;resize:vertical;line-height:1.5;margin-bottom:16px;">{{ mbNoteBottom }}</textarea>'
