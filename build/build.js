...
const arenaCalPanel = '<sc-if value="{{ showArenaCal }}">'
  + '<div style="background:var(--panel);border:1px solid var(--border);border-radius:18px;padding:18px 20px;margin-bottom:22px;">'
    + '<div style="display:flex;align-items:flex-start;justify-content:space-between;gap:12px;flex-wrap:wrap;margin-bottom:14px;">'
      + '<div><div style="font-family:\'Archivo\';font-weight:900;font-size:20px;letter-spacing:-.01em;">Arena Calendar &#183; {{ arenaCalLabel }}</div>'
      + '<div style="display:flex;gap:16px;margin-top:8px;font-size:11.5px;color:var(--muted);"><span style="display:inline-flex;align-items:center;gap:5px;"><span style="width:11px;height:11px;border-radius:3px;background:var(--volt);display:inline-block;"></span>Class</span><span style="display:inline-flex;align-items:center;gap:5px;"><span style="width:11px;height:11px;border-radius:3px;background:#2B3242;display:inline-block;"></span>Venue Booking</span></div></div>'
...
const registerRecap = '<sc-if value="{{ showRegister }}">'
  + '<div style="margin-bottom:22px;">'
    + '<div style="display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;margin-bottom:6px;">'
      + '<div style="font-family:\'Archivo\';font-weight:800;font-size:18px;">Attendance Recap</div>'
      ...
      + '<sc-for list="{{ g.classes }}" as="cl"><div style="background:var(--panel);border:1px solid var(--border);border-radius:14px;overflow:hidden;margin-bottom:12px;">'
        ...
        + '<sc-for list="{{ cl.participants }}" as="p"><div style="padding:11px 16px;border-bottom:1px solid var(--border);">'
          + '<div style="display:flex;align-items:center;gap:12px;">'
            + '<div style="flex:1;min-width:0;"><div style="font-weight:700;font-size:13.5px;">{{ p.name }}</div><div style="font-size:11.5px;color:var(--muted);margin-top:2px;font-family:\'JetBrains Mono\';">&#128222; {{ p.phone }} &#183; &#9993; {{ p.email }}</div><sc-if value="{{ p.hasAddon }}"><div style="margin-top:5px;"><span style="background:color-mix(in srgb,var(--amber) 16%,transparent);color:var(--amber);border-radius:100px;padding:2px 10px;font-size:10.5px;font-weight:700;">&#128717; Add-on: {{ p.addonLabel }}</span></div></sc-if></div>'
            ...
            + '<sc-if value="{{ registerCanCheck }}"><button onclick="{{ p.toggle }}" style="flex-shrink:0;border:1px solid var(--border2);background:{{ p.attBg }};color:{{ p.attFg }};border-radius:7px;padding:6px 18px;font-size:11.5px;font-weight:700;cursor:pointer;">Present</button></sc-if>'
          ...
          + '</div>'
          + '<sc-if value="{{ registerCanNote }}"><input value="{{ p.note }}" onchange="{{ p.saveNote }}" placeholder="Note (e.g. permission, late)…" style="width:100%;box-sizing:border-box;margin-top:8px;background:var(--bg);border:1px solid var(--border2);border-radius:8px;padding:7px 10px;color:var(--text);font-family:\'Hanken Grotesk\';font-size:12px;outline:0;"></sc-if>'
    ...
    + '<sc-if value="{{ hasRegisterTotals }}"><div style="background:var(--panel);border:1px solid var(--border);border-radius:14px;padding:16px 18px;margin-top:18px;">'
      + '<div style="font-family:\'Archivo\';font-weight:800;font-size:15px;margin-bottom:4px;">&#9201; Coach Teaching Total &#183; this month</div>'
      + '<sc-if value="{{ registerHoursOff }}"><div style="font-size:11.5px;color:var(--amber);margin-bottom:8px;">Total hours become available after the DB migration (checkout_at) is run.</div></sc-if>'
      + '<sc-for list="{{ registerCoachTotals }}" as="t"><div style="display:flex;align-items:center;justify-content:space-between;gap:12px;padding:8px 0;border-bottom:1px solid var(--border);font-size:13px;">'
        ...
        + '<div style="color:var(--muted);font-size:12px;">{{ t.completed }}/{{ t.sessions }} sessions done &#183; <span style="color:var(--text);font-weight:800;font-size:14px;">{{ t.hours }}</span></div>'
      + '</div></sc-for>'
    + '</div></sc-if>'
    + '<sc-if value="{{ noRegister }}"><div style="background:var(--panel);border:1px solid var(--border);border-radius:14px;padding:34px 24px;text-align:center;color:var(--muted);">No participant data for this period yet.</div></sc-if>'
  + '</div>'
  + '</sc-if>';
...
const checkoutReminder = '<sc-if value="{{ hasPendingCheckout }}">'
  + '<div style="background:rgba(176,113,10,.12);border:1px solid var(--amber);border-radius:14px;padding:14px 16px;margin-bottom:18px;">'
    + '<div style="font-weight:800;font-size:14px;color:var(--amber);">&#9200; Remember to Check Out</div>'
    + '<div style="font-size:12.5px;color:var(--muted);margin:4px 0 10px;">This class is finished but not yet checked out:</div>'
...
const coachSessPanel = '<sc-if value="{{ showCoachSess }}">'
  + '<div style="background:var(--panel);border:1px solid var(--border);border-radius:18px;padding:18px 20px;margin-bottom:16px;">'
    + '<div style="display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;margin-bottom:6px;">'
      + '<div style="font-family:\'Archivo\';font-weight:800;font-size:18px;">Coach Session Recap &#183; {{ coachSessMonthLabel }}</div>'
      + '<div style="display:flex;gap:9px;align-items:center;flex-wrap:wrap;">'
        ...
        + '<button onclick="{{ exportMonthly }}" style="background:var(--volt);border:0;color:#fff;border-radius:10px;padding:9px 15px;font-family:\'Archivo\';font-weight:800;font-size:12.5px;cursor:pointer;">&#128196; Print PDF (full)</button>'
      + '</div>'
    + '</div>'
    + '<div style="font-size:12px;color:var(--muted);margin-bottom:12px;">Done = coach check-in &#183; Completed = check-out. Counted automatically from coach attendance.</div>'
    + '<sc-if value="{{ coachSessHoursOff }}"><div style="background:rgba(176,113,10,.14);border:1px solid var(--amber);border-radius:10px;padding:9px 13px;font-size:12px;color:var(--amber);margin-bottom:12px;">The &#8220;Total hours&#8221; column becomes available after the DB migration (checkout_at) is run.</div></sc-if>'
    + '<div style="overflow-x:auto;"><div style="min-width:680px;">'
      + '<div style="display:grid;grid-template-columns:1.5fr .8fr .8fr .8fr .8fr .9fr 1.2fr;gap:8px;padding:8px 10px;font-size:10.5px;font-weight:800;color:var(--muted);letter-spacing:.04em;text-transform:uppercase;border-bottom:1px solid var(--border);">'
        + '<div>Coach</div><div style="text-align:center;">Scheduled</div><div style="text-align:center;">{{ csConducted }}</div><div style="text-align:center;">Completed</div><div style="text-align:center;">Pax</div><div style="text-align:center;">Total hours</div><div>Status</div>'
      + '</div>'
      + '<sc-for list="{{ coachSessRows }}" as="r"><div style="display:grid;grid-template-columns:1.5fr .8fr .8fr .8fr .8fr .9fr 1.2fr;gap:8px;padding:11px 10px;font-size:13px;border-bottom:1px solid var(--border);align-items:center;">'
        ...
        + '<div style="text-align:center;font-variant-numeric:tabular-nums;font-weight:700;">{{ r.pax }}</div>'
      ...
      + '</div></sc-for>'
      + '<sc-if value="{{ noCoachSess }}"><div style="padding:30px;text-align:center;color:var(--muted);">No sessions recorded this month.</div></sc-if>'
    + '</div></div>'
  + '</div></sc-if>';
...
const membersScreen = '<sc-if value="{{ s.members }}"><div style="max-width:900px;margin:0 auto;">'
  ...
  + '<sc-if value="{{ noMembers }}"><div style="' + cardBox + 'padding:44px 24px;text-align:center;color:var(--muted);">No participants for this period yet.</div></sc-if>'
  + '</div></sc-if>';
...
const venueCard = '<div style="' + cardBox + 'padding:16px 18px;">'
  + '<div style="display:flex;align-items:flex-start;justify-content:space-between;gap:12px;flex-wrap:wrap;">'
    ...
    + '<sc-if value="{{ b.hasBookingCoach }}"><div style="font-size:12px;color:var(--muted2);margin-top:2px;">Coach: {{ b.bookingCoach }}</div></sc-if>'
  ...
  + '</div>'
  + '<sc-if value="{{ b.showCoachDropdown }}"><div style="margin-top:13px;padding-top:13px;border-top:1px solid var(--border);display:flex;align-items:center;gap:10px;flex-wrap:wrap;">'
    + '<span style="font-size:11.5px;font-weight:600;color:var(--muted);white-space:nowrap;">Coach (optional)</span>'
    + '<select onchange="{{ b.setCoach }}" style="flex:1;min-width:150px;background:var(--bg);border:1px solid var(--border2);border-radius:9px;padding:9px 11px;color:var(--text);font-family:\'Hanken Grotesk\';font-size:12.5px;cursor:pointer;"><option value="">No coach</option><sc-for list="{{ b.coachIdOpts }}" as="o"><option value="{{ o.id }}" selected="{{ o.picked }}">{{ o.label }}</option></sc-for></select>'
  + '</div></sc-if>'
  + '<sc-if value="{{ b.showSetCoachBtn }}"><div style="margin-top:13px;padding-top:13px;border-top:1px solid var(--border);">'
    + '<button onclick="{{ b.startSetCoach }}" style="background:transparent;border:1px dashed var(--border2);color:var(--muted);border-radius:9px;padding:8px 14px;font-size:12px;font-weight:700;cursor:pointer;" style-hover="border-color:var(--volt);color:var(--volt);">+ Set coach</button>'
  + '</div></sc-if>'
...
const packageScreen = '<sc-if value="{{ s.packageorders }}"><div style="max-width:1000px;margin:0 auto;">'
  ...
  + '<div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:16px;">'
    + '<input id="pkgSearchInput" value="{{ pkgSearchVal }}" oninput="{{ setPkgSearch }}" placeholder="Search participant name or order code…" style="flex:1;min-width:220px;background:var(--bg);border:1px solid var(--border2);border-radius:11px;padding:11px 14px;color:var(--text);font-family:\'Hanken Grotesk\';font-size:14px;outline:0;box-sizing:border-box;">'
  ...
  + '<sc-if value="{{ pkgHasOrders }}"><div style="' + cardBox + 'overflow:hidden;"><div style="overflow-x:auto;"><div style="min-width:820px;">'
    + '<div style="display:grid;grid-template-columns:150px 1.2fr 1.6fr 78px 118px 92px;background:var(--panel2);color:var(--muted2);font-size:10.5px;font-weight:700;letter-spacing:.05em;text-transform:uppercase;">'
      + '<div style="padding:12px 14px;">Order Code</div><div style="padding:12px 14px;">Participants</div><div style="padding:12px 14px;">Package</div><div style="padding:12px 14px;">Sessions</div><div style="padding:12px 14px;">Purchase Date</div><div style="padding:12px 14px;">Status</div>'
    + '</div>'
    ...
    + '</sc-for>'
  + '</div></div></div></sc-if>'
  + '<sc-if value="{{ pkgNoOrders }}"><div style="' + cardBox + 'padding:44px 24px;text-align:center;color:var(--muted);">No matching package orders.</div></sc-if>'
+ '</div></sc-if>';
...
const rescheduleScreen = '<sc-if value="{{ s.reschedule }}"><div style="max-width:1200px;margin:0 auto;">'
  ...
  + '</div>'
  + '<div style="font-size:13px;color:var(--muted);margin-bottom:16px;">Pick a participant to reschedule &#183; {{ rschCountLabel }}</div>'
  + '<div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:16px;">'
    + '<input id="rschSearchInput" value="{{ rschSearchVal }}" oninput="{{ setRschSearch }}" placeholder="Search name, booking code, or phone…" style="flex:1;min-width:240px;background:var(--bg);border:1px solid var(--border2);border-radius:11px;padding:11px 14px;color:var(--text);font-family:\'Hanken Grotesk\';font-size:14px;outline:0;box-sizing:border-box;">'
  ...
  + '<sc-if value="{{ rschHasRows }}"><div style="' + cardBox + 'overflow:hidden;"><div style="overflow-x:auto;"><div style="min-width:960px;">'
    + '<div style="display:grid;' + rschGrid + 'background:var(--panel2);color:var(--muted2);font-size:10.5px;font-weight:700;letter-spacing:.05em;text-transform:uppercase;">'
      + '<div style="padding:12px 14px;">Booking Code</div><div style="padding:12px 14px;">Class</div><div style="padding:12px 14px;">Schedule</div><div style="padding:12px 14px;">Name</div><div style="padding:12px 14px;">Phone</div><div style="padding:12px 14px;">Status</div><div style="padding:12px 14px;text-align:right;">Action</div>'
    + '</div>'
    ...
    + '</sc-for>'
  + '</div></div></div></sc-if>'
  + '<sc-if value="{{ rschNoRows }}"><div style="' + cardBox + 'padding:44px 24px;text-align:center;color:var(--muted);">No matching bookings.</div></sc-if>'
+ '</div></sc-if>';
...
const vcGrid = 'grid-template-columns:52px 84px 1.4fr 132px 184px 66px 128px 152px;';
const validateScreen = '<sc-if value="{{ s.validatecoach }}"><div style="max-width:1200px;margin:0 auto;">'
  ...
  + '<sc-if value="{{ vcHasRows }}"><div style="' + cardBox + 'overflow:hidden;"><div style="overflow-x:auto;"><div style="min-width:1040px;">'
    + '<div style="display:grid;' + vcGrid + 'background:var(--panel2);color:var(--muted2);font-size:10.5px;font-weight:700;letter-spacing:.05em;text-transform:uppercase;white-space:nowrap;">'
      + '<div style="padding:12px 14px;text-align:center;">{{ vcColNo }}</div><div style="padding:12px 14px;">{{ vcColTime }}</div><div style="padding:12px 14px;">{{ vcColSession }}</div><div style="padding:12px 14px;">{{ vcColType }}</div><div style="padding:12px 14px;">{{ vcColCoaches }}</div><div style="padding:12px 14px;text-align:right;">{{ vcColPax }}</div><div style="padding:12px 14px;">Status</div><div style="padding:12px 14px;text-align:right;">Action</div>'
    + '</div>'
    ...
    + '<div style="display:grid;' + vcGrid + 'border-top:1px solid var(--border);align-items:center;">'
      ...
      + '<div title="{{ r.label }}" style="padding:12px 14px;font-weight:700;font-size:13px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;min-width:0;">{{ r.label }}</div>'
      + '<div style="padding:12px 14px;font-size:12px;color:var(--muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;min-width:0;">{{ r.typeLabel }}</div>'
      + '<div title="{{ r.coaches }}" style="padding:12px 14px;font-size:12.5px;color:{{ r.coachesCol }};white-space:nowrap;overflow:hidden;text-overflow:ellipsis;min-width:0;">{{ r.coaches }}</div>'
      + '<div style="padding:12px 14px;text-align:right;font-family:\'JetBrains Mono\';font-size:12.5px;font-weight:700;color:{{ r.paxCol }};">{{ r.pax }}</div>'
      ...
      + '<div style="padding:9px 14px;"><button onclick="{{ r.act }}" disabled="{{ r.actDisabled }}" title="{{ r.actTitle }}" style="width:100%;box-sizing:border-box;display:block;text-align:center;background:{{ r.actBg }};border:0;color:{{ r.actFg }};border-radius:9px;padding:8px 6px;font-family:\'Archivo\';font-weight:800;font-size:11px;cursor:{{ r.actCursor }};text-transform:uppercase;letter-spacing:.02em;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{{ r.actLabel }}</button></div>'
    + '</div>'
    + '</sc-for>'
  + '</div></div></div></sc-if>'
...
// ---- Gym unit view (Arena<->Gym switcher): month calendar + day classes + client list. ----
const gymGrid7 = 'display:grid;grid-template-columns:repeat(7,1fr);gap:6px;';
const gymCliCols = 'grid-template-columns:54px 1.6fr 1fr 96px 130px;';
const gymScreen = '<sc-if value="{{ s.gymview }}"><div style="max-width:1100px;margin:0 auto;">'
  + '<div style="display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;margin-bottom:4px;">'
    + '<div style="font-family:\'Archivo\';font-weight:800;font-size:22px;">20FIT Gym</div>'
    + '<span style="font-size:10px;font-weight:700;letter-spacing:.14em;text-transform:uppercase;color:var(--cyan);border:1px solid rgba(77,212,242,.4);background:rgba(77,212,242,.08);border-radius:100px;padding:5px 11px;">Unit &#183; Gym</span>'
  + '</div>'
  + '<div style="font-size:13px;color:var(--muted);margin-bottom:16px;">Jadwal kelas &amp; daftar klien unit Gym.</div>'
  + '<div style="display:grid;grid-template-columns:1.25fr 1fr;gap:16px;align-items:start;">'
    + '<div style="' + cardBox + 'padding:16px;">'
      + '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;">'
        + '<button onclick="{{ gymPrev }}" style="' + selStyle + '">&lsaquo;</button>'
        + '<div style="font-family:\'Archivo\';font-weight:800;font-size:15px;">{{ gymCalLabel }}</div>'
        + '<button onclick="{{ gymNext }}" style="' + selStyle + '">&rsaquo;</button>'
      + '</div>'
      + '<div style="' + gymGrid7 + 'margin-bottom:6px;"><sc-for list="{{ gymDow }}" as="d"><div style="text-align:center;font-size:10px;font-weight:700;color:var(--muted2);text-transform:uppercase;">{{ d.lbl }}</div></sc-for></div>'
      + '<div style="' + gymGrid7 + '"><sc-for list="{{ gymCells }}" as="c">'
        + '<sc-if value="{{ c.blank }}"><div></div></sc-if>'
        + '<sc-if value="{{ c.day }}"><button onclick="{{ c.pick }}" style="aspect-ratio:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:3px;border-radius:9px;border:1px solid {{ c.border }};background:{{ c.bg }};color:{{ c.col }};cursor:pointer;font-family:\'Hanken Grotesk\';font-weight:700;font-size:13px;"><span>{{ c.day }}</span><sc-if value="{{ c.dot }}"><span style="width:5px;height:5px;border-radius:50%;background:var(--cyan);"></span></sc-if></button></sc-if>'
      + '</sc-for></div>'
    + '</div>'
    + '<div style="' + cardBox + 'padding:16px;">'
      + '<div style="font-size:11px;font-weight:700;letter-spacing:.05em;color:var(--muted2);text-transform:uppercase;margin-bottom:10px;">{{ gymDayLabel }}</div>'
      + '<sc-if value="{{ gymDayHas }}"><sc-for list="{{ gymDay }}" as="k">'
        + '<div style="display:flex;align-items:center;gap:11px;padding:10px 0;border-bottom:1px solid var(--border);">'
          ...
          + '<div style="flex:1;min-width:0;"><div style="font-weight:700;font-size:13px;">{{ k.type }}</div><div style="font-size:11.5px;color:var(--muted);">{{ k.time }} {{ k.end }} &#183; {{ k.coach }}</div></div>'
          + '<span style="font-family:\'JetBrains Mono\';font-size:12px;font-weight:700;color:var(--muted);">{{ k.paxLabel }}</span>'
        + '</div>'
      + '</sc-for></sc-if>'
      + '<sc-if value="{{ gymDayEmpty }}"><div style="text-align:center;color:var(--muted);font-size:12.5px;padding:24px 0;">Tidak ada kelas Gym pada tanggal ini.</div></sc-if>'
    + '</div>'
  + '</div>'
  + '<div style="margin-top:18px;">'
    + '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;flex-wrap:wrap;gap:8px;"><div style="font-family:\'Archivo\';font-weight:800;font-size:16px;">Klien Gym</div><div style="font-size:12px;color:var(--muted);">{{ gymClientsTotal }} klien &#183; {{ gymClientsActive }} aktif 30 hari</div></div>'
...
const rentersScreen = '<sc-if value="{{ s.renters }}"><div style="max-width:760px;margin:0 auto;">'
  ...
  + '<sc-if value="{{ hasVenueRenters }}"><div style="' + cardBox + 'overflow:hidden;">'
    + '<sc-for list="{{ venueRenters }}" as="r"><div style="display:flex;align-items:center;gap:13px;padding:13px 18px;border-bottom:1px solid var(--border);">'
      ...
      + '<div style="flex:1;min-width:0;"><div style="font-weight:700;font-size:14.5px;">{{ r.name }}</div><div style="font-size:11.5px;color:var(--muted);">Last rental: {{ r.lastLabel }}</div></div>'
    ...
    + '</div></sc-for>'
  + '</div></sc-if>'
  + '<sc-if value="{{ noVenueRenters }}"><div style="' + cardBox + 'padding:44px 24px;text-align:center;color:var(--muted);">No arena booking data for this period yet.</div></sc-if>'
  + '</div></sc-if>';
