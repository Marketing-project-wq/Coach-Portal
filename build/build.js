...
// Make each calendar day clickable -> show that day's classes
template = template.replace('<div style="text-align:center;padding:12px 4px;border-radius:11px;background:{{ d.bg }};border:1px solid {{ d.border }};">', '<div onclick="{{ d.pick }}" style="text-align:center;padding:12px 4px;border-radius:11px;background:{{ d.bg }};border:1px solid {{ d.border }};cursor:pointer;">');
// Inject "Monitoring Kelas per Bulan" panel at the end of the coach dashboard
const monthlyPanel = '<div style="background:var(--panel);border:1px solid var(--border);border-radius:18px;padding:20px;margin-top:16px;"><div style="font-weight:800;font-family:\'Archivo\';font-size:16px;margin-bottom:16px;">Monitoring Kelas per Bulan · {{ monthlyYear }}</div><div style="display:grid;grid-template-columns:repeat(12,1fr);gap:6px;align-items:end;"><sc-for list="{{ monthly }}" as="m"><div style="text-align:center;"><div style="font-size:13px;font-weight:800;font-family:\'Archivo\';margin-bottom:6px;color:{{ m.col }};">{{ m.count }}</div><div style="height:{{ m.h }}px;min-height:4px;background:{{ m.bar }};border-radius:5px;"></div><div style="font-size:10px;color:var(--muted);margin-top:6px;">{{ m.month }}</div></div></sc-for></div></div>';
template = template.replace(/<\/div>(\s*)<\/sc-if>(\s*)(<!-- ===== CLASS DETAIL)/, monthlyPanel + '</div>$1</sc-if>$2$3');
// Inject the "Review Peserta" screen (sibling screen in the scroll area)
const reviewsScreen = '<sc-if value="{{ s.reviews }}"><div style="max-width:820px;margin:0 auto;"><div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px;margin-bottom:20px;"><div><div style="font-family:\'Archivo\';font-weight:800;font-size:22px;">Review Peserta</div><div style="color:var(--muted);font-size:13.5px;margin-top:3px;">Ulasan peserta untuk kelas yang mereka ikuti.</div></div><div style="display:flex;gap:12px;"><div style="background:var(--panel);border:1px solid var(--border);border-radius:14px;padding:12px 18px;text-align:center;min-width:96px;"><div style="font-family:\'Archivo\';font-weight:900;font-size:26px;color:var(--amber);">{{ reviewAvg }}</div><div style="font-size:11px;color:var(--muted);">Rata-rata &#9733;</div></div><div style="background:var(--panel);border:1px solid var(--border);border-radius:14px;padding:12px 18px;text-align:center;min-width:96px;"><div style="font-family:\'Archivo\';font-weight:900;font-size:26px;">{{ reviewCount }}</div><div style="font-size:11px;color:var(--muted);">Total review</div></div></div></div><div style="background:var(--panel);border:1px solid var(--border);border-radius:14px;padding:14px 18px;margin-bottom:16px;display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;"><div style="font-size:13px;color:var(--muted);">Link review untuk peserta:<br><span style="color:var(--volt);font-family:\'JetBrains Mono\';font-size:13px;">{{ reviewLink }}</span></div><button onclick="{{ copyReviewLink }}" style="background:var(--raised);border:1px solid var(--border2);color:var(--text);border-radius:9px;padding:9px 14px;font-weight:700;font-size:12.5px;cursor:pointer;">Salin Link</button></div><sc-for list="{{ reviews }}" as="rv"><div style="background:var(--panel);border:1px solid var(--border);border-radius:14px;padding:16px 18px;margin-bottom:12px;"><div style="display:flex;align-items:center;justify-content:space-between;gap:10px;"><div style="font-weight:700;font-size:14px;">{{ rv.name }} <span style="color:var(--muted2);font-weight:400;font-size:12px;">&#183; {{ rv.cls }}{{ rv.coachSuffix }}</span></div><div style="color:var(--amber);font-size:15px;letter-spacing:2px;">{{ rv.stars }}</div></div><div style="color:var(--text);font-size:13.5px;margin-top:8px;">{{ rv.comment }}</div><div style="color:var(--muted2);font-size:11.5px;margin-top:8px;">{{ rv.date }}</div></div></sc-for><sc-if value="{{ noReviews }}"><div style="background:var(--panel);border:1px solid var(--border);border-radius:14px;padding:40px;text-align:center;color:var(--muted);">Belum ada review. Bagikan link di atas ke peserta Anda &#10024;</div></sc-if></div></sc-if>';
